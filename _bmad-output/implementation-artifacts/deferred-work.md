...
## Deferred from: code review of 3-2-trang-chu-hom-nay-streak (2026-07-09)

- `.saveError` (danger) và `.nextHint` (secondary) bị selector `.article p` (specificity 0-1-1) đè `color` → render nhầm `--color-text-primary` thay vì danger/secondary. Pre-existing từ story 3.1 (dev đã tự flag trong Completion Notes 3.2). Story 3.2 đã né đúng trap này cho `.weekDone` bằng scope `.completion .weekDone` (0-2-0). Fix trivial cùng khuôn: đổi thành `.completion .saveError` / `.completion .nextHint`. Không ảnh hưởng AC nào của 3.2. Evidence: src/features/lesson/LessonPage.module.css:140,147.
- Nhãn "Tuần N" ("Xong Tuần {weekNumber}", "Tuần N · X/M bài") và chuỗi "Giai đoạn 1" là hardcode/không mang ngữ cảnh phase. weekNumber đánh số per-phase (content/index.ts) nên khi phase 2 ship (FR-2/SM-3: thêm phase = import + register), "Xong Tuần 1" / "Tuần 1 · X/M" của hai phase sẽ nhập nhằng với người dùng. Hôm nay chỉ có 1 phase và microcopy khớp spec verbatim (UX-DR10) nên không phải vi phạm 3.2 — tổng quát hóa khi thêm phase. Evidence: src/features/roadmap/HomePage.tsx:296,302; src/features/lesson/LessonPage.tsx:87.
- `role="status"` live region mount có điều kiện (`weekDone !== null` / `saveError`) thay vì luôn hiện diện. Một số screen reader chỉ đọc tin cậy khi text ĐỔI trong live region đã có sẵn; chèn cả element live region có thể bị bỏ lỡ. Khớp convention `saveError` sẵn có (story 3.1) nên nhất quán — nhưng convention tự nó mong manh. Giải quyết một lần cho toàn app (live region persistent bọc các toast transient). Evidence: src/features/lesson/LessonPage.tsx:183-187, 190-194.
